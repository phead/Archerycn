...
app.config(function($routeProvider) {
  ...
  $routeProvider.when('/live1', {
    templateUrl: 'demo/live1.html',
    reloadOnSearch: false
  });
});
...
app.controller('VideoController', function($scope, video) {
   ...
   */
  $scope.videos = {
    first: 'http://ggqsmbd8v8apgutqm2b.exp.bcelive.com/lss-ghqsgfxxgrm72546/live.m3u8',
    second: 'http://ggqsmbd8v8apgutqm2b.exp.bcelive.com/lss-ghhqm6wetn1re03i/live.m3u8'
  };

  $scope.currentVideo = $scope.videos.first;

  $scope.next = function() {
    if ($scope.currentVideo == $scope.videos.first) {
      $scope.currentVideo = $scope.videos.second;
    } else {
      $scope.currentVideo = $scope.videos.first;
    }
    video.addSource('application/x-mpegURL', $scope.currentVideo, true);
  };
  // Add some video sources for the player!
  video.addSource('application/x-mpegURL', $scope.videos.first);
});